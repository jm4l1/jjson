...
namespace jjson
{
	...
	jjson_str_t value::to_string(bool pretty_format, int indent_level) const
	{
		...
		case value_type::STRING:
			...
			return string_color + string_string + to_escaped_string(*jimpl_->string_value, JJSON_ESCAPE_CHARACTERS) + string_quote +
				   color_reset;
		case value_type::INT:
		...
		case value_type::ARRAY:
			...
			for (auto itr = jimpl_->array_value->begin() + 1; itr != jimpl_->array_value->end(); ++itr)
			{
				array_string += ",";
		...
		case value_type::OBJECT:
			...
			for (auto itr = jimpl_->object_value->begin(); itr != jimpl_->object_value->end();
				 // ++itr
			)
			{
				...
				object_string += JJSON_DQUOTE + itr->first + JJSON_DQUOTE + (pretty_format ? " : " : ":") +
								 itr->second.to_string(pretty_format, indent_level);
	...
	{
		if (string_object[0] != '-' && std::isdigit(string_object[0]) == 0)
		{
			return value();
		}
		if (string_object[0] == '0' && string_object.length() != 1)
		{
			return value();
		}
	...
	value value::parse_as_float(const jjson_str_t& string_object)
	{
		...
		if (std::count(string_object.begin(), string_object.end(), '.') > 1)
		{
			return value();
		}
		size_t str_pos = string_object.find_first_of("Ee");
	...
	value value::parse_from_string(const jjson_str_t& string_object)
	{
		...
		else if (!string_object.empty() && string_object[0] == '"' && string_object[string_object.length() - 1] == '"')
		{
			if (string_object == JJSON_DQUOTE) return value();
		...
		else if (!string_object.empty() && (string_object.find_first_of(".") != jjson_str_t::npos))
		{
